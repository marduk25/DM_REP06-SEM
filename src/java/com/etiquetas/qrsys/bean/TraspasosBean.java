...
public class TraspasosBean implements Serializable {
    ...
    public void buscarTraspasosEntrada() {
        ...
        try {
            ...
            dao.listarFacturasTraspasosEntrada(fechaInicial, fechaFinal, departamento);
            listaFacturasV = new ArrayList<>();
            FacturaDao fDao = new FacturaDaoImp();
            listaFacturasV = fDao.listaTraspasoEntrada();
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public void listarSeriesimpresion(String nofactura) {
        if (!nofactura.isEmpty()) {
            ...
            listaSeriesImpresion = sDao.listaSeriesImprTrasEntrada(nofactura);
    ...
    public void imprimirEtiquetas() {
       ...
       public void listarSeriesimpresionSalida(String nofactura) {
        if (!nofactura.isEmpty()) {
            SerieDao sDao = new SerieDaoImp();
            listaSeriesImpresion = sDao.listaSeriesImprTrasSalida(nofactura);
            
        }
    }
    
    public void imprimirEtiquetasSalida() {
        this.contador = 0;
        this.noFacturaImpresion = null;
        this.listaSeriesEstado1 = new ArrayList<>();
        for (int i = 0; i < listaSeriesImpresion.size(); i++) {
        ...
        listarSeriesimpresionSalida(noFacturaImpresion);
        //**ABRIMOS EL DIALOGO PARA IMPRIMIR**//
        RequestContext.getCurrentInstance().execute("PF('dlgImpresion').show()");
        if (this.contador == listaSeriesImpresion.size()) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "SISTEMA DE ETIQUETAS", "Selecciona etiquetas"));
    ...
    public void buscarTraspasosSalida() {
        ...
        try {
            ...
            String fechaInicial = format.format(f1);
            String fechaFinal = format.format(f2);
            dao.listarFacturasTraspasosSalida(fechaInicial, fechaFinal, departamento);
            listaFacturasV = new ArrayList<>();
            FacturaDao fDao = new FacturaDaoImp();
            listaFacturasV = fDao.listaTraspasoSalida();
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public void regresarEstadoSalidaFalse() {
        String dato = listaSeriesEstado1.toString().replace("[", "").replace("]", "");
        String[] info = dato.split(",");
        for (int i = 0; i < listaSeriesEstado1.size(); i++) {
            SerieDao sDao = new SerieDaoImp();
